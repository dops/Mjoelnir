...
class TCPDF {
	 ...
	 */
	public function setController($pnum, $resetmargins=false) {
	...
	public function lastPage($resetmargins=false) {
		$this->setController($this->getNumPages(), $resetmargins);
	}
	...
	public function startPage($orientation='', $format='', $tocpage=false) {
		...
		if ($this->numpages > $this->page) {
			...
			$this->setController($this->page + 1);
			$this->SetY($this->tMargin);
			return;
		}
		// start a new page
				...
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						if (!isset($dom[$trid]['startpage'])) {
							...
							$this->setController($dom[$trid]['startpage']);
						}
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		switch($tag['value']) {
			case 'tr': {
				...
				$this->setController($dom[($dom[$key]['parent'])]['endpage']);
				if ($this->num_columns > 1) {
